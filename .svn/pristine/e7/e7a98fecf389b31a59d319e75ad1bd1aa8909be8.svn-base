...
public class DbHelper extends SQLiteOpenHelper{
   ...
   private static final String ENABLE_FOREIGN_KEY="PRAGMA FOREIGN_KEYS=\"ON\"";
	...
	private int fare[][]= new int[134][];  
	private Context context;
    private int STOP_SIZE = 134;
	...
	private void read_line(SQLiteDatabase db) {
      try{
		 ...
		 db.execSQL("insert into line('id','line_name') values('8','NA');");
		}catch(SQLiteException e){
			e.printStackTrace();
		}
	}
	...
	public HashMap<String, String> getStopsColorMap(ArrayList<Vertex> nodes) {
		...
		try {
		Cursor result = db.rawQuery("select stop_name,color_id from stops", null);
		if(result.moveToFirst()) {
			do {
				String stop_name = result.getString(0);
  ...
  public ArrayList<Edge> getEdgeList(){
	  ...
	  SQLiteDatabase db = this.getReadableDatabase();
	  try {
			Cursor result = db.rawQuery("select source,destination from stop_sequence", null);
			if(result.moveToFirst()) {
				do {
					String source = result.getString(0);
					String destination = result.getString(1);
					Edge edge = new Edge("",new Vertex(source, source),new Vertex(destination, destination), 1);
					list.add(edge);
				}while(result.moveToNext());
			}
			}catch (SQLiteException e) {
